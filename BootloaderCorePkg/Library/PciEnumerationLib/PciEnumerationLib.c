  ...
  )
{
  ...
  if (RootBridgeInfoHob != NULL) {
    DEBUG ((DEBUG_INFO, "PciRootBridgeResourceInfoHob: Revision 0x%X, RootBridgeCount 0x%X\n",
...
/**
  Build PCI Root Bridge Resource Info HOB
  ...
  )
{
  ...
  while ((CurrentLink != NULL) && (CurrentLink != &RootBridge->ChildList)) {
    Root = PCI_IO_DEVICE_FROM_LINK (CurrentLink);
    BuildPciRootBridgeInfoHob (Root, RootBridgeInfoHob, RootBridgeCount);
  ...
  )
{
  ...
  BuildPciInfoHob (RootBridge, RootBridgeCount);
