  ...
  )
{
  ...
  case TT_ATTR_INDX_MEMORY_WRITE_BACK:
    ...
    break;
  default:
    DEBUG ((DEBUG_ERROR, "PageAttributeToGcdAttribute: PageAttributes:0x%lX not supported.\n", PageAttributes));
    ASSERT (0);
  ...
  )
{
  ...
  if (ArmReadCurrentEL () == AARCH64_EL2) {
    ...
    if (MaxAddress < SIZE_4GB) {
      ...
      DEBUG ((DEBUG_ERROR, "ArmConfigureMmu: The MaxAddress 0x%lX is not supported by this MMU configuration.\n", MaxAddress));
